...
def analyze_among_among5(dataset, model, temperature):
    save_dir = './result_data/%s_%s_%s/' % (dataset, model, temperature)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    problem_dic = {}
    with open('log/record/dataset_%s_model_%s_topn_5_temperature_%s.0.log_%s' % (dataset, model, temperature, 0), 'r') as f:
        for line in f.readlines():
            content = json.loads(line)
            name = content['name']
            if name not in problem_dic:
...
if __name__ == "__main__":
    ...
    if args.option == 'R1':
        analyze_among_among5(args.dataset, args.model, args.temperature)
    elif args.option == 'R2':
